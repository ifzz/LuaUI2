...
void ImeInput::MoveImeWindow(HWND window_handle, HIMC imm_context) {
	...
	int height = caret_rect_.bottom - caret_rect_.top;
	if (system_caret_) {
	...
	int width = caret_rect_.right - caret_rect_.left;
	height = caret_rect_.bottom - caret_rect_.top;
...
void ImeInput::GetCaret(HIMC imm_context, LPARAM lparam, ImeComposition* composition) 
{
  ...
  case LANG_JAPANESE:
	  ...
	  if (lparam & GCS_COMPATTR) {
		  ...
		  if (attribute_size > 0) {
			  ...
			  attribute_data = new char[attribute_size];
			  if (attribute_data) {
...
bool ImeInput::GetString(HIMC imm_context, WPARAM lparam, int type, ImeComposition* composition) 
{
	...
	if (lparam & type) {
		...
		if (string_size > 0) {
			...
			composition->ime_string.resize(string_length + 1);
			//wchar_t *string_data = WriteInto(&composition->ime_string,
			//	string_length + 1);
			wchar_t *string_data = &(composition->ime_string[0]);
			if (string_data) {
...
void ImeInput::EnableIME(HWND window_handle, const RECT& caret_rect, bool complete) 
{
